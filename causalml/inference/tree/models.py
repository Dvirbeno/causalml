...
class UpliftTreeClassifier:
    ...
    def fit(self, X, treatment, y, obligatory_features=None):
    ...
    def growDecisionTreeFrom(self, X, treatment, y, evaluationFunction, max_depth=10,
        ...
        drawn_features = list(np.random.choice(a=range(columnCount), size=max_features, replace=False))
        drawn_features = list(set(drawn_features + self.obligatory_features))
        for col_idx, col in enumerate(drawn_features):
            # print(f"Examining feature #{col_idx} (out of {max_features})")
            columnValues = X[:, col]
            # unique values
            ...
            # nunique = len(lsUnique)
            for val_idx, value in enumerate(lsUnique):
                # print(f"    Examining Split #{val_idx} (out of {nunique})")
...
class UpliftRandomForestClassifier:
    ...
    def fit(self, X, treatment, y, obligatory_features=None):
        ...
        self.uplift_forest = (
            ...
            (delayed(self.bootstrap)(X, treatment, y, tree, obligatory_features) for tree in self.uplift_forest)
        )
    ...
    @staticmethod
    def bootstrap(X, treatment, y, tree, obligatory_features=None):
        bt_index = np.random.choice(len(X), len(X))
        x_train_bt = X[bt_index]
        y_train_bt = y[bt_index]
        treatment_train_bt = treatment[bt_index]
        tree.fit(X=x_train_bt, treatment=treatment_train_bt, y=y_train_bt, obligatory_features=obligatory_features)
        return tree
